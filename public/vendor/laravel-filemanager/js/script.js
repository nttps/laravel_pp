...
function loadFolders() {
    performLfmRequest('folders', {}, 'html')
        .done(function (data) {
            $('#tree').html(data);
            $(document).trigger("treeview")
            loadItems();
        });
}

$(document).on("treeview", function () {
    $("#m_tree_1").jstree({
            core: {
                themes: {
                    responsive: !1
        ...
        $("#m_tree_1").on("select_node.jstree", function (e, t) {
...
function loadItems() {
    ...
    performLfmRequest('jsonitems', {
        ...
        .done(function (data) {
            ...
            if (getPreviousDir() == '') {
                $('#to-previous').addClass('hidden');
            } else {
                $('#to-previous').removeClass('hidden');
            }
            setOpenFolders();
        })
        .always(function () {
...
function fileView(file_url, timestamp) {
    bootbox.dialog({
        ...
        message: $('<img>')
            .addClass('img img-fluid text-center')
